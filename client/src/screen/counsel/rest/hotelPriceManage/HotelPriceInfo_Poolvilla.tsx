...
function HotelCostList_Poolvilla({ hotelCost, title, highlightedCosts, isPoolVilla = false }: HotelCostListPoolvillaProps) {
...
export default function HotelPriceInfo_Poolvilla({
	...
	const [preStayFinalSearchResult, setPreStayFinalSearchResult] = useState<string>('');
					...
					if (inputDefault && inputDefault.costByRoomType && Array.isArray(inputDefault.costByRoomType)) {
						const matchingRoom = inputDefault.costByRoomType.find((rt: any) => {
							...
							if (preStaySelectedPeriodType) {
								let hasPeriod = false;
								if (preStaySelectedPeriodType === '2+2' && rt.twoTwoDayCost) hasPeriod = true;
								if (preStaySelectedPeriodType === '1+3' && rt.oneThreeDayCost) hasPeriod = true;
								if (preStaySelectedPeriodType === '3' && rt.threeDayCost && rt.threeDayCost !== '') hasPeriod = true;
								if (preStaySelectedPeriodType === '4' && rt.fourDayCost) hasPeriod = true;
						...
						if (matchingRoom) {
							...
							if (!finalResult) {
								...
								let priceText = '';
								if (preStaySelectedPeriodType === '2+2' && matchingRoom.twoTwoDayCost) {
									priceText = `2+2: ${formatPrice(matchingRoom.twoTwoDayCost)}`;
								} else if (preStaySelectedPeriodType === '1+3' && matchingRoom.oneThreeDayCost) {
									priceText = `1+3: ${formatPrice(matchingRoom.oneThreeDayCost)}`;
								} else if (preStaySelectedPeriodType === '3' && matchingRoom.threeDayCost && matchingRoom.threeDayCost !== '') {
									priceText = `3only: ${formatPrice(matchingRoom.threeDayCost)}`;
								} else if (preStaySelectedPeriodType === '4' && matchingRoom.fourDayCost) {
								...
								finalResult = `예약유형: ${cost.reserveType === 'earlyPeriod' ? '얼리버드' : cost.reserveType === 'default' ? '기본' : cost.reserveType || '-'} / 예약기간: ${cost.reservePeriod || '-'} / 룸타입별 요금: ${matchingRoom.roomType || '-'} - ${priceText}`;
							}
						}
					}
				} catch (e) {
		...
		poolVilla1HotelCost.costInput.forEach((cost: any, idx: number) => {
			...
			if (matches && (poolVilla1SelectedRoomType || poolVilla1SelectedPeriodType)) {
				try {
					...
					if (inputDefault && inputDefault.costByRoomType && Array.isArray(inputDefault.costByRoomType)) {
						const matchingRoom = inputDefault.costByRoomType.find((rt: any) => {
							if (poolVilla1SelectedRoomType && rt.roomType !== poolVilla1SelectedRoomType) {
								return false;
							}
							
							if (poolVilla1SelectedPeriodType) {
								let hasPeriod = false;
								if (poolVilla1SelectedPeriodType === '2+2' && rt.twoTwoDayCost) hasPeriod = true;
								if (poolVilla1SelectedPeriodType === '1+3' && rt.oneThreeDayCost) hasPeriod = true;
								if (poolVilla1SelectedPeriodType === '3' && rt.threeDayCost && rt.threeDayCost !== '') hasPeriod = true;
								if (poolVilla1SelectedPeriodType === '4' && rt.fourDayCost) hasPeriod = true;
						...
						if (matchingRoom) {
							...
							if (!finalResult) {
								...
								let priceText = '';
								if (poolVilla1SelectedPeriodType === '2+2' && matchingRoom.twoTwoDayCost) {
									priceText = `2+2: ${formatPrice(matchingRoom.twoTwoDayCost)}`;
								} else if (poolVilla1SelectedPeriodType === '1+3' && matchingRoom.oneThreeDayCost) {
									priceText = `1+3: ${formatPrice(matchingRoom.oneThreeDayCost)}`;
								} else if (poolVilla1SelectedPeriodType === '3' && matchingRoom.threeDayCost && matchingRoom.threeDayCost !== '') {
									priceText = `3only: ${formatPrice(matchingRoom.threeDayCost)}`;
								} else if (poolVilla1SelectedPeriodType === '4' && matchingRoom.fourDayCost) {
									priceText = `4only: ${formatPrice(matchingRoom.fourDayCost)}`;
								} else {
									const parts: string[] = [];
									if (matchingRoom.twoTwoDayCost) parts.push(`2+2: ${formatPrice(matchingRoom.twoTwoDayCost)}`);
