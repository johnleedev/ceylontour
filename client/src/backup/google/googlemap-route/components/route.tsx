...
export type RouteProps = {
  ...
  onRouteStepsChange?: (steps: any[]) => void;
};

const Route = (props: RouteProps) => {
  const {apiClient, origin, destination, routeOptions, onRouteStepsChange} = props;
  ...
  // 상위 컴포넌트로 routeSteps 전달
  useEffect(() => {
    if (route && route.legs && route.legs[0] && route.legs[0].steps && onRouteStepsChange) {
      onRouteStepsChange(route.legs[0].steps);
    }
  }, [route, onRouteStepsChange]);
